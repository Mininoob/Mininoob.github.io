...
- Personal project (user-provided, 2026-07, detail updated): **Polymage** — low-poly mage action-RPG on a custom C++20 engine written from scratch on Vulkan 1.3 (MoltenVK on macOS). Infinite seed-based streaming voxel world at 0.25 m resolution, rendered by a GPU compute-shader raymarcher rather than triangle meshes (caves, overhangs, micro-voxel look); chunks generate and page in around the player on a worker-thread pool. Authoritative deterministic 60 Hz ECS (EnTT + Jolt physics) consuming only input command structs, never input devices — replay-verifiable and structured for future networked multiplayer; CI replays a command stream and asserts identical results every commit. Game layer: procedurally generated biomes, rivers, dungeons, towns; data-driven spell system with damage-over-time and slow effects; gold, loot drops, inventory and stat-modifying equipment; boss fights with health-threshold phases, damage-based threat table, telegraphed heavy attacks, summoned adds, arena hazards. Renderer: ambient occlusion, contact shadows, day/night atmospheric sky, reflective water, dynamic point lights (torches/lava/crystals), HDR bloom + ACES tonemapping. Builds and unit-test-green on Windows and macOS via CMake presets. In development; no public link yet. Docked in Bay P-01 (condensed on the site).
...
- Resume: published 2026-07 at /Joshua_M_Le_Resume.pdf (phone-free version from ~/Resumes/Joshua_M_Le_Resume.pdf; lists Houston, Texas + email + mininoob.github.io). Earlier path /JoshuaLe_Resume.pdf was retired the same day.
